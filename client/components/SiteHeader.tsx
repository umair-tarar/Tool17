    ...
    <header className="fixed inset-x-4 top-4 z-50 rounded-full border border-white/70 bg-white/75 shadow-lg shadow-indigo-100/60 backdrop-blur-md sm:inset-x-6 lg:inset-x-8">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8">
        ...
        <div className="hidden items-center gap-3 md:flex">
          <Link to="/access" className="shine-button rounded-full px-3 py-2 text-sm font-semibold text-[#26375e] transition hover:text-[#273baf] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#5164d8]">Login</Link>
          <Link to="/access" className="shine-button rounded-full bg-[#3349be] px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-[#263aa5] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#5164d8] focus-visible:ring-offset-2">Get Started</Link>
        </div>
        ...
        {mobileMenuOpen && <nav id="mobile-navigation" className="absolute inset-x-5 top-full mt-1 rounded-2xl border border-slate-200 bg-white p-3 shadow-xl shadow-slate-900/10 md:hidden" aria-label="Mobile navigation">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2 border-t border-slate-100 pt-3">
            <Link to="/access" onClick={() => setMobileMenuOpen(false)} className="shine-button rounded-full border border-slate-200 px-3 py-2.5 text-center text-sm font-bold text-slate-700">Login</Link>
            <Link to="/access" onClick={() => setMobileMenuOpen(false)} className="shine-button rounded-full bg-[#3349be] px-3 py-2.5 text-center text-sm font-bold text-white">Get Started</Link>
          </div>
        </nav>}
      </div>
    </header>
